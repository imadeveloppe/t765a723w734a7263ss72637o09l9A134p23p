...
class Centre extends CI_Model {
        ...
        public function verified( $idCentre = null )
        {   
            $id = ( $idCentre ) ? $idCentre : $this->session->id ;
        ...
        public function connecte()
        {
            ...
            if( $num_rows > 0 ){
                ...
                $dataSession = array(
                        ...
                        'allowProfToSendMsg' => $result[0]->allowProfToSendMsg,
                        'allowParentToSendMsg' => $result[0]->allowParentToSendMsg,
                        'niveau' => 1,
                        'logged_in' => true,
                        'subAdmin' => false
                ); 
        ...
        {                      
            ...
            if( $num_rows > 0 ){
                ...
                $mail->IsSMTP();
                $mail->Host       = "tawassolapp.com";
                $mail->SMTPDebug  = 0;                                    
                $mail->SMTPAuth   = true;                  
                $mail->Host       = "mail.tawassolapp.com"; 
                $mail->Port       = 26;                    
                $mail->Username   = "mail@tawassolapp.com"; 
                $mail->Password   = "tawassolapp2017"; 

                $mail->SetFrom('contact@tawassolapp.com', 'Tawassol App'); 
                $mail->AddReplyTo("contact@tawassolapp.com","Tawassol App");
        ...
        {
            $query = $this->db->select('C.nom, C.email, C.tel, C.about, C.reglement_interieur, C.vacances_scolaires,C.niveau,C.photo, V.intitule as ville'); 
        ...
        public function update()
        {
            ...
            if(!empty($_FILES['about']['name'])){
                ...
                $content_dir = base_url().'assets/upload/ecoles/'; 
                $tmp_file = $_FILES['about']['tmp_name']; 

                $config['upload_path'] ='assets/upload/ecoles/'; 
                $config['allowed_types'] = 'pdf';
                $this->load->library('upload', $config);

                $_FILES['about']['name'] = preg_replace("/[^a-zA-Z0-9.]/", "", $_FILES['about']['name']);
                
                if($this->upload->do_upload('about')){ 
                        $array['about'] = $this->upload->data('file_name');    
                }else{
                    echo $this->upload->display_errors();
                }  
            } 
        ...
        {   
            ...
            if( empty($existeReglement_interieur) ){
                $array['reglement_interieur'] = ''; 
            }
            if( empty($existeVacances_scolaires) ){
                $array['vacances_scolaires'] = ''; 
            }

            if(!empty($_FILES['about']['name'])){

                $content_dir = base_url().'assets/upload/docs/'; 
                $tmp_file = $_FILES['about']['tmp_name']; 
            ...
            if(!empty($_FILES['reglement_interieur']['name'])){

                $content_dir = base_url().'assets/upload/docs/'; 
                $tmp_file = $_FILES['reglement_interieur']['tmp_name']; 

                $config['upload_path'] ='assets/upload/docs/'; 
                $config['allowed_types'] = 'pdf';
                $this->load->library('upload', $config);

                $_FILES['reglement_interieur']['name'] = preg_replace("/[^a-zA-Z0-9.]/", "", $_FILES['reglement_interieur']['name']);
                
                if($this->upload->do_upload('reglement_interieur')){ 
                        $array['reglement_interieur'] = $this->upload->data('file_name');    
                }else{
                    echo $this->upload->display_errors();
                }  
            } 

            if(!empty($_FILES['vacances_scolaires']['name'])){

                $content_dir = base_url().'assets/upload/docs/'; 
                $tmp_file = $_FILES['vacances_scolaires']['tmp_name']; 

                $config['upload_path'] ='assets/upload/docs/'; 
                $config['allowed_types'] = 'pdf';
                $this->load->library('upload', $config);

                $_FILES['vacances_scolaires']['name'] = preg_replace("/[^a-zA-Z0-9.]/", "", $_FILES['vacances_scolaires']['name']);
                
                if($this->upload->do_upload('vacances_scolaires')){ 
                        $array['vacances_scolaires'] = $this->upload->data('file_name');    
                }else{
                    echo $this->upload->display_errors();
                }  
            } 

            if( $this->db->update($this->table, $array, array('id' => $this->session->id)) ){
                return true;
            }
        }
        ...
        public function GetInfoCentre( $idCentre )
        {
        ...
        public function NotifRepWhenCentreIsDeleted( $idCentre ){
        ...
        public function NotifRepWhenCentreIsActivated( $idCentre ){
            $query = $this->db->select('R.token, C.nom'); 
            $query = $this->db->from($this->table.' as C'); 
            $query = $this->db->join('representant as R', 'C.id_rep = R.id'); 
            $query = $this->db->where(array('C.id'=>$idCentre));
            ...
            switch ( $_POST['type'] ) {
                case 'prof': 
                    $this->db->update(
                        $this->table, 
                        array(
                            "allowProfToSendMsg" => $_POST['value']
                        ), 
                        array('id' => $this->session->id)
                    );
                    $this->session->set_userdata('allowProfToSendMsg', $_POST['value']);
                    break;
                ...
                case 'parent': 
                    ...
                    $this->db->update(
                        $this->table, 
                        array(
                            "allowParentToSendMsg" => $_POST['value']
                        ), 
                        array('id' => $this->session->id)
                    );

                    $this->session->set_userdata('allowParentToSendMsg', $_POST['value']);
                    break;  
            } 
        }
